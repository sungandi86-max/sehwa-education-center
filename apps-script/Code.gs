...
function appendQrAttendance(req, eventId, staffId, signatureRecord) {
  ...
  const data = {
    ...
    "교직원ID": staffId,
    staffId,
    "성명": req.staffName || req.name || stringOf(staff, ["성명", "name", "staffName"]),
    "소속부서": req.department || stringOf(staff, ["소속부서", "부서", "department"]),
    "직책": req.position || stringOf(staff, ["직책", "직위", "position"]),
    "참석일시": now,
    "참석방법": "QR",
    "상태": "출석완료",
    signatureId: signatureRecord.signatureId || "",
