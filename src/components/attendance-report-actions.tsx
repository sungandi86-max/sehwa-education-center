  ...
  const downloadReport = async () => {
    ...
    try {
      ...
      if (!response.ok || !result.success || !result.data?.fileUrl) {
        setMessage(result.message || "최종 서명부를 생성하지 못했습니다. 잠시 후 다시 시도해주세요.");
        return;
      }

      window.open(result.data.fileUrl, "_blank", "noopener,noreferrer");
      setMessage("최종 서명부가 생성되었습니다.");
    } catch {
      setMessage("최종 서명부를 생성하지 못했습니다. 잠시 후 다시 시도해주세요.");
    } finally {
  ...
  return (
    <div className="grid gap-2 sm:grid-cols-3">
      ...
      <button type="button" onClick={downloadReport} disabled={isDownloading} className="btn-primary">
        <Download size={18} />
        {isDownloading ? "최종 서명부를 생성하고 있습니다..." : "최종 명단 다운로드"}
      </button>
