...
export default async function TrainingDetailPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const eventMaterials = materials.filter((material) => material.eventId === eventId && material.공개여부 !== false);
    ...
    <div className="space-y-8">
      ...
      <section className="quiet-card overflow-hidden">
        <div className="bg-gradient-to-br from-white via-white to-brand-50/70 p-7 md:p-8">
          <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
            <div className="min-w-0">
              ...
              <h1 className="mt-4 text-3xl font-semibold tracking-tight text-brand-900 md:text-4xl">{event.제목}</h1>
              {event.설명 ? <p className="mt-4 max-w-3xl text-base leading-7 text-slate-600">{event.설명}</p> : null}
            </div>
        ...
        <div className="grid gap-4 border-t border-slateblue-100 p-7 md:grid-cols-3 md:p-8">
          <InfoItem label="일시" value={`${formatDateTime(event.시작일시)} - ${formatDateTime(event.종료일시)}`} />
          <InfoItem label="장소" value={event.장소} />
          <InfoItem label="담당부서" value={event.담당부서} />
        </div>
      </section>
      ...
      <section className="quiet-card overflow-hidden">
        <div className="border-b border-slateblue-100 px-7 py-5 md:px-8">
          ...
          <p className="mt-1 text-sm leading-6 text-slate-500">담당자가 Google Sheet에 등록한 자료 링크입니다.</p>
        </div>
            ...
            eventMaterials.map((material) => (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-brand-600">{material.자료유형}</p>
                    <h3 className="mt-2 text-lg font-semibold text-brand-900">{material.제목}</h3>
                  </div>
